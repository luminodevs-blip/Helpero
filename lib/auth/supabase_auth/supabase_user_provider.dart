...
class HelperoSupabaseUser extends BaseAuthUser {
  HelperoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> helperoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HelperoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
